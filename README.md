...
<details>
<summary>97. Які існують стратегії розділення коду (code-splitting) у Vue.js додатку?</summary>

#### Vue.js

Code-splitting допомагає зменшити початковий розмір бандла і завантажувати
тільки потрібний код. Основні стратегії у Vue.js:

1. Dynamic Imports для компонентів

```JavaScript
const AsyncComponent = () => import('./components/MyComponent.vue')
```

- Компонент завантажується лише при потребі (lazy loading).
...
- Маршрути завантажуються окремо, коли користувач переходить на сторінку.

3. Webpack / Vite code splitting через import()

- Можна динамічно імпортувати утиліти, бібліотеки, частини стану.

4. Split Chunks / Optimization

- Виносити загальні залежності у vendor bundle, щоб кешувати між сторінками.

- Перевага: швидший initial load, менше пам’яті у браузері, кращий UX.

</details>

<details>
<summary>98. На що слід звертати увагу при lazy loading компонентів у Vue.js?</summary>

#### Vue.js

1. **Async компонент повинен бути функцією**

```JavaScript
const AsyncComp = () => import('./MyComponent.vue')
```

2. **Обробка стану завантаження і помилок**
...
- Кожен async компонент створює окремий chunk.

- Важливо не розбивати занадто дрібно, щоб не створювати надто багато запитів.

4. **SEO та SSR**

- У SSR lazy loading компоненти рендеряться на сервері, якщо потрібно, або через
  suspense.

5. **Кешування**

- Використовувати браузерне кешування або prefetch для часто використовуваних
  компонентів.

- Перевага: зменшує initial bundle, прискорює перший рендер, покращує UX.

</details>

<details>
<summary>99. Які техніки використовуються для керування маршрутизацією у великих Vue.js додатках?</summary>

#### Vue.js

Для великих додатків важливо організувати маршрути так, щоб код залишався чистим
і продуктивним. Основні техніки:

1. Модульна структура маршрутів

- Розбивати маршрути на окремі файли за фічами або модулями:
...
2. Lazy loading компонентів для маршрутів
...
- Структурувати маршрути вкладеними для складних розділів додатку.

4. Named routes та route meta

- Для зручного переходу та обмеження доступу:
...
- beforeEach, beforeEnter для авторизації, логування, лоадерів.

6. Dynamic routing

- Генерація маршрутів на основі даних з API для CMS-подібних додатків.

</details>

<details>
<summary>100. Як вибір стилю коду та архітектури впливає на масштабованість Vue.js додатків?</summary>

#### Vue.js

Стиль коду і архітектура критично впливають на масштабованість:

1. **Модульна структура компонентів**

- Розділення на маленькі, повторно використовувані компоненти полегшує підтримку
  та тестування.

2. **Composition API vs Options API**

- Composition API краще підходить для великих проєктів: логіка фіч зберігається
  разом, легше створювати reusable functions.

3. **Глобальний стан**

- Використання Pinia або Composition API для управління станом дозволяє уникнути
  хаосу у великих додатках.

4. **Routing та lazy loading**

- Модульні маршрути та lazy loading компонентів зменшують розмір initial bundle
  і покращують продуктивність.

5. **Стилі**

- Scoped CSS, CSS Modules, або Tailwind/Utility-first підхід зменшують конфлікти
  стилів і спрощують рефакторинг.

6. **Тестування та документація**

- Чітка архітектура дозволяє легше писати unit/integration тести і підтримувати
  документацію.

- Вплив: легше додавати фічі, менше технічного боргу, краща продуктивність і
  підтримка команди.
